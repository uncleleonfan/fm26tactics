...
**GSC 页面索引验证结果（2026-09-07）**：

| 状态 | 数量 | 判定 |
|---|---|---|
| Page with redirect | 100 | 旧 locale URL 已识别 301，权重合并中，非错误 |
| Alternate page with proper canonical tag | 3 | canonical/hreflang 正常 |
| Discovered - currently not indexed | 11 | 信任度不足，暂未抓取 |
| Crawled - currently not indexed | 9 | 已抓取但未收录，价值信号弱 |

- **「重复网页」类目已消失**——索引不再持有旧版重复页判定，9-4 locale 清理已在索引层面生效，「索引缓存旧版」的顾虑排除；
- 剩余短板收敛为 **20 页未索引**（站点年轻/信任度），即「低价值内容」拒批的现实土壤，非技术缺陷；
- **对 §4 时机的影响**：原「30 天等待」可缩短——待未索引页多数入索引、总索引量企稳后即可重申请。

**未索引 URL 排查（2026-09-07，Discovered 10 例）**：

构成：3 战术详情 + 3 博客 + 1 指南 + 1 角色页 + about/contact。验证：
- 线上抽检 4 URL 均 `200 OK`；
- 战术详情页初始 HTML 服务端渲染完整（h1/正文/FAQ JSON-LD/related 内链均在，`ssr:false` 时代问题已根治）；
- 全部 URL 在 sitemap 内，导航/首页/面包屑/related 内链覆盖；
- mdx 源文件存在，内容量正常（10KB+）。

**结论：无技术缺陷，纯抓取预算/信任度分配**（站点上线约 3 周）。无需代码侧动作。处置：GSC URL 检查逐个「请求编入索引」（日配额约 10 个）；1~2 周后复查，入索引 + 总索引量企稳即重提 AdSense。未索引状态本身不阻塞 AdSense 审核（审核看页面可访问性与内容，非索引状态）。

**Crawled - currently not indexed 9 例排查（2026-09-07）**：全部为重定向变体——6 个旧 locale URL（tr/fr/de，Sep 3~5 仍在重抓）+ 3 个 apex 域名 URL（非 www）。线上验证 tr/de/apex 均返回 308 永久重定向 → en/www 最终页（Google 对 308 与 301 等同处理）。变体不索引是正确终态，随最终页入索引自动收敛，**零整改项**。唯一真实内容页为此状态者：`/tactics/4-4-2-wing-play`，并入请求索引清单（合计 11 个 URL，约两天配额）。
