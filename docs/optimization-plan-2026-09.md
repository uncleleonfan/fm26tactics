...
> **状态（2026-08-29）**：N13 ✓（llms.txt 修正 www 规范 + 补 /meta、/best、guides 亮点链接）｜N14 → 观察（9 月跳出率）｜N15 ✓（调研完成 → `docs/fmf-research-2026-08.md`：程序化生成不可行（无规范/只读逆向/零写入先例），推荐"人工导出预打包 .fmf 库"分阶段方案，阶段 1 需正版游戏人工导出 Top 10）｜N16 ✓（dual-phase 指南全文验证**零 Builder 承诺**（IP/OOP 指游戏内 Tactics 屏幕），与 revert 功能无脱节，无需改动）｜N17 ✓（代码体检无硬伤：recharts 已 dynamic+ssr:false、无裸 img、字体自托管、GA 延迟加载；已清理 2 个死广告域名 preconnect；**无需立项**，桌面排名更可能是内容/外链因素）｜N18 → 观察（9 月数据）｜N19 → 暂缓（Q4）
...
*v2.1 更新：2026-08-29（新增 §2b 多语言策略评估，D2 改为渐进式方案）｜ 状态：Batch 1 ✓ + Batch 2 ✓ + Batch 3 可执行项 ✓（2026-08-29：N13/N15/N16/N17 完成，N14/N18 观察中，N19 Q4 再议）｜ 下一步：9 月底数据复盘（§5 看板）+ N15 阶段 1 决策（是否人工导出 .fmf）*
