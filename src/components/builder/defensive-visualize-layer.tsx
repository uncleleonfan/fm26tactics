...
/** Metric bars + findings for the defensive scenario (glass panel overlay). */
export function DefensiveMetricsPanel({ result }: { result: DefensiveResult }) {
  const t = useTranslations("visualize");
  const { metrics, findings } = result;

  const bars = [
    { key: "pressure", value: metrics.pressureOnBall, higherIsBetter: true },
    { key: "compact", value: metrics.compactness, higherIsBetter: true },
    { key: "ballSide", value: metrics.ballSideCover, higherIsBetter: true },
    { key: "behind", value: metrics.spaceInBehind, higherIsBetter: false },
  ];

  const barColor = (value: number, higherIsBetter: boolean) => {
    const good = higherIsBetter ? value >= 0.55 : value <= 0.45;
    const bad = higherIsBetter ? value < 0.3 : value > 0.6;
    return good ? "#00E676" : bad ? "#FF5252" : "#FFB300";
  };

  return (
    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 w-max max-w-[calc(100%-1.5rem)]">
      <div className="rounded-xl border border-[#1C2436] bg-background-secondary/95 backdrop-blur-xs shadow-[0_4px_24px_rgba(0,0,0,0.5)] px-3 py-2 animate-fade-in">
        <div className="flex items-center gap-3">
          {bars.map((bar) => (
            <div key={bar.key} className="flex flex-col items-center gap-1 w-11">
              <span className="text-[8px] font-semibold uppercase tracking-wide text-text-muted">
                {t(`defensiveMetrics.${bar.key}`)}
              </span>
              <div className="w-full h-1 rounded-full bg-[#1C2436] overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-300"
                  style={{
                    width: `${Math.round(bar.value * 100)}%`,
                    backgroundColor: barColor(bar.value, bar.higherIsBetter),
                  }}
                />
              </div>
            </div>
        ...
        {findings.length > 0 && (
          <ul className="mt-2 space-y-1 border-t border-[#1C2436] pt-1.5">
            {findings.map((f) => (
              <li key={f.id} className="flex items-start gap-1.5 text-[9px] leading-snug">
                <span
                  aria-hidden
