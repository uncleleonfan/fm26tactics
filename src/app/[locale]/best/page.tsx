...
interface RankMeta {
  badge: string; color: string; icon: React.ReactNode; reason: string;
}
const rankMeta: Record<string, RankMeta> = {
  "4-2-3-1-gegenpress": { badge: "#1 Meta Pick", color: "text-amber-400 bg-amber-500/10 border-amber-500/30", icon: <Trophy className="w-4 h-4 text-amber-400" />, reason: "The most consistent FM26 formation. High press, quick transitions, works at every level." },
  "3-5-2-catenaccio": { badge: "#2 Defensive King", color: "text-blue-400 bg-blue-500/10 border-blue-500/30", icon: <Shield className="w-4 h-4 text-blue-400" />, reason: "Most-read tactic on our site. Impregnable defense that still scores on the counter." },
  "4-3-3-tiki-taka": { badge: "#3 Possession Master", color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30", icon: <Star className="w-4 h-4 text-emerald-400" />, reason: "Total control. 65%+ possession with elite teams. Beautiful football." },
  "4-4-2-wing-play": { badge: "#4 Classic Power", color: "text-orange-400 bg-orange-500/10 border-orange-500/30", icon: <Zap className="w-4 h-4 text-orange-400" />, reason: "Old-school 4-4-2 updated for FM26. Devastating wing play and strike partnerships." },
  "3-4-3-control-possession": { badge: "#5 Modern Hybrid", color: "text-purple-400 bg-purple-500/10 border-purple-500/30", icon: <Star className="w-4 h-4 text-purple-400" />, reason: "Total football reimagined. Overloads everywhere, perfect for squad depth." },
  "4-1-2-1-2-diamond": { badge: "#6 Midfield Overload", color: "text-cyan-400 bg-cyan-500/10 border-cyan-500/30", icon: <Star className="w-4 h-4 text-cyan-400" />, reason: "Dominate the center. Perfect when you have a world-class attacking midfielder." },
  "4-3-3-fluid-counter": { badge: "#7 Speed Demon", color: "text-red-400 bg-red-500/10 border-red-500/30", icon: <Zap className="w-4 h-4 text-red-400" />, reason: "Lightning transitions. Best counter-attacking 4-3-3 for pace-based squads." },
  "3-5-2-counter-attack": { badge: "#8 Underdog Special", color: "text-teal-400 bg-teal-500/10 border-teal-500/30", icon: <Shield className="w-4 h-4 text-teal-400" />, reason: "The ultimate small-club tactic. Hit bigger teams where it hurts." },
};
...
export default async function BestTacticsPage({ params }: { params: { locale: string } }) {
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      { "@type": "Question", name: "What are the best FM26 tactics?", acceptedAnswer: { "@type": "Answer", text: "The best FM26 tactics are the 4-2-3-1 Gegenpress (best overall), 3-5-2 Catenaccio (best defensive), and 4-3-3 Tiki-Taka (best possession). The 4-2-3-1 Gegenpress is the most consistent formation, working at every club level with its high-press, quick-transition style." } },
      { "@type": "Question", name: "What is the strongest formation in FM26?", acceptedAnswer: { "@type": "Answer", text: "The 4-2-3-1 Gegenpress is widely considered the strongest formation in FM26 due to its balance of defensive solidity and attacking threat. It provides two holding midfielders for defensive cover, three attacking midfielders for creativity, and a lone striker who benefits from overloads." } },
      { "@type": "Question", name: "Which FM26 tactic is best for beginners?", acceptedAnswer: { "@type": "Answer", text: "The 4-4-2 Wing Play and 3-5-2 Catenaccio are the best FM26 tactics for beginners. Both feature simple role assignments, clear tactical instructions, and don't require complex player instructions." } },
      { "@type": "Question", name: "How do I choose the best FM26 tactic for my team?", acceptedAnswer: { "@type": "Answer", text: "Match your tactic to your squad strength: strong wingers → 4-2-3-1 or 4-4-2, strong midfield → 4-1-2-1-2 Diamond or 4-3-3, strong defense → 3-5-2, underdog team → 3-5-2 Counter-Attack." } },
    ],
  };
                ...
                <Link key={tactic.slug} href={`/tactics/${tactic.slug}`} className="block glass-card group hover:border-primary/30 transition-all duration-300">
                  <div className="p-5 sm:p-6">
                    <div className="flex items-center gap-3 mb-4 flex-wrap">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${meta.color}`}>{meta.icon}{meta.badge}</span>
                      <span className="text-xs font-mono font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-md">{tactic.formation}</span>
                      <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${diff.className}`}>{diff.label}</span>
                      <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${styleColors[tactic.style]}`}>{styleLabels[tactic.style]}</span>
                    </div>
                    <h2 className="text-lg font-semibold text-text-primary mb-2 group-hover:text-primary transition-colors">{tactic.title}</h2>
                    <p className="text-sm text-text-secondary leading-relaxed mb-3">{tactic.description}</p>
                    <p className="text-xs text-text-muted italic mb-3">{b("whyRanks")}{index + 1}: {meta.reason}</p>
